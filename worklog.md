...
## 2026-08-05 — v4.0.14: Mengeneinheit Stück→Packungen (Spec-Befund)

Andreas-Befund bestätigt durch BFS-Spec: AMOR liefert Sollmengen in GRUNDEINHEITEN ("Basismengeneinheit Stück: eine 20er-Packung von X wird als 20 Stück von X bestellt" — einmalige Klinik-Konfiguration; "Stück pro Packung ist nötig, wenn Sollmengen in Stück geliefert werden"). Botmatiq las sie als Packungen → Auftrag 10000019: 620 "Packungen"/23 Behälter statt real ~11 Packungen. Auch die Mirta-500-"Testmenge" war real 5×100er-Packungen — Felde-Rückfrage obsolet.

v4.0.14 (e2a46af, Mac-Relay 12×4KB-Chunks + SHA-Gate; Suite 1227/1227): Config Botmatiq:Amor:OrderQuantityUnit=pack|piece (Default pack); piece-Ingest: Quantity=ceil(Grundmenge/PackageSize), additive Spalten BaseUnitQuantity+PackSizeAtIngest (Snapshot); Artikel-Lookup mit PZN-Formen-Fallback; Rückmeldung in Stück (Soll exakt originaltreu über gesplittete Zeilen — letzte Zeile trägt Rest; Ist=Packungen×Snapshot, Aufrundung → Fehlercode 1); UI Packungen + "= N Stk. (à P)"; Spec_Addendum_OrderQuantityUnit.md. update414-Block: .env piece setzen, Bestandsaufträge per SQL umrechnen, Behälterpläne löschen (Composer plant nach), offene Läufe nur LISTEN (Storno im UI — Spaltennamen von pick_batches nicht verifiziert, kein Blind-SQL).

**Nachtrag v4.0.15 (05.08., Mac-Relay f020b22):** Beleg-Konsistenz für den Stück-Modus: Lieferschein-HTML zeigt je Position „4 = 40 Stk." und in der Summary „(= N Stk.)"; Wannenlieferschein-PDF (PerTote) „4 (= 40 Stk.)" via PackSizeByPzn-Map aus den OrderItems (additiv im ReportContext). Ausgewiesen wird die GELIEFERTE Stückzahl (Packungen × Snapshot), nicht die bestellte — der Test deckte auf, dass das HTML zunächst die Bestellmenge (32) zeigte; Fix auf geliefert (40), konsistent zum PDF; die Abweichung zur Bestellung dokumentiert die Rückmeldung (Fehlercode 1). pack-Aufträge bleiben byte-identisch. Test Kernkette_PieceModus_Belege_WeisenStueckAus, Suite 1228/1228. Kommt beim nächsten Auto-Update mit (kein eigener Rollout-Block).

**Relay-Lektion (Chunk-Gate, verbindlich):** Beim v4.0.15-Transfer wurden 2 von 4 Chunks beim Übertragen verfälscht (1 Byte fehlte / 4 zu viel) → git am „corrupt patch". Verfahren verschärft: (1) JEDEN Chunk nach write_file sofort per `md5 -q` gegen den Sandbox-`md5sum` prüfen; (2) bei Mismatch 500-Byte-Split-Bisektion (`split -b 500` beidseitig, MD5-Vergleich — wegen Offset-Shift zählt das ERSTE abweichende Stück), dann `head -c` truncate + Rest-Append statt Komplett-Neuschreiben; (3) SHA-256 von .b64 UND dekodiertem Patch als hartes Gate VOR git am. Zweiter Anlauf sauber: f020b22 auf main + Tag v4.0.15, Windows Publish lief an.

## 2026-08-05 — v4.0.16: Rechtssichere s/w-Belege (Freigabe Andreas)

Beide Belege (Lieferschein-HTML + Wannenlieferschein-PDF) überarbeitet nach Rechtsrecherche (§ 31 Abs. 1+2, § 22 Abs. 2, § 26 Abs. 2 ApBetrO; ABDA-LL Krankenhausversorgung; Muster-Versorgungsvertrag § 14 ApoG): Unterschriftspflicht liegt auf der ANFORDERUNG der Station (AMOR-Auftrag), nicht auf dem Abgabebeleg → Unterschriftsfelder ersatzlos raus (inkl. falscher „§14 ApoBetrO"-Referenz), stattdessen Dokumentationsblock: elektronisch erstellt/ohne Unterschrift gültig, Abgabe gem. § 31 Abs. 1 auf elektronische Anforderung (AMOR-Nr. + Anforderungsstelle aus persistiertem Kopfsatz), Pick-Doku je Position, Behälterkontrolle im Vier-Augen-Prinzip (Bediener + Vision) mit Capture-ID und Ablageort (Kliniknetz, keine Cloud), Aufbewahrung § 22 (5 Jahre). Vollständig Graustufen (Status via Symbole/Text, Vision-Badge s/w-Rahmen, lokalisiert DE/EN/FR). Absenderblock statt „Botmatiq Krankenhausapotheke": neues Tenant-Feld delivery_note_sender (mehrzeilig, additiv, Fallback CustomerName/Location) — Merseburg: Zentralapotheke / Carl-von-Basedow-Klinikum Saalekreis gGmbH (ohne Namensnennung der Leitung — keine Rechtspflicht); Hersteller nur als Fußzeilen-Vermerk. Lieferschein weist GELIEFERTE Mengen aus: offene Positionen Menge 0 + „Rückstand (N)" + Sammelvermerk, Wannen-PDF mit Rückstands-Kasten auf dem Deckblatt. Verfalldatum je Position nur bei belegten Daten (batch_tracking aus Wareneingang/SecurPharm), sonst entfällt die Spalte. Interne Betriebsdaten (Regal/Band/Pickzeiten) raus. Suite 1229/1229. Commits 3e795fc (+ fcb6fd6 Fix: TenantController.UpdateTenant übernahm delivery_note_sender nicht), Tag v4.0.16 → nächstes Auto-Update. sender-merseburg-block.ps1 setzt den Absender per SQL (Spalten-Guard: erst nach Update ausführen; erwartete Länge 89). Offen: Vision-Bild-Retention 30 Tage vs. Beleg-Verweis (Empfehlung ≥ 5 Jahre für Merseburg); FR-Beleg referenziert deutsche ApBetrO — vor Nancy CSP-Referenz prüfen.

**Relay 2.0 (Standard ab jetzt, ersetzt Chunk-Tippen):** Sandbox kann GitHub-READS via Vault-GIT_PAT (fetch/ls-remote/clone), Writes blockt der Git-Proxy weiterhin (auch REST, auch .github). Push-Weg: git format-patch → SendUserFile → device_commit_files nach ~/tmp_transfer (byte-genau, SHA stimmte im ersten Versuch) → SHA-256-Gate auf dem Mac → git am -3 → push. Ordner-Freigabe ~/tmp_transfer einmalig je Session (Dialog braucht Andreas am Mac). Sandbox-Checkout danach per fetch+reset --hard auf FETCH_HEAD syncen (PAT nur transient in URL, nie in .git/config lassen).

## 2026-08-05 — v4.0.17: Vision-Langzeitarchiv auf dem Botmatiq-Server (Entscheidung Andreas)

5-Jahres-Aufbewahrung der Wannen-Capture-Bilder (Beleg-Bildnachweis § 22 ApBetrO) auf admin.botmatiq.de statt IPC/externer HDD. Server (Tag deploy-superadmin-20260805b, success): POST/stats/download `/api/ingest/v1/vision-archive*` (Sync-Token tenant-gebunden, SHA-256 Pflicht+serverseitig verifiziert, temp+rename, Upsert UNIQUE tenant+capture_id), Ablage `/mnt/data/botmatiq/vision-archive/<tenant>/<yyyy>/<mm>/capture-<uuid>.jpg` im Server-Backup-Umfang, KEINE Retention; Compose-Volume idempotent im Deploy-Workflow gepatcht. Core v4.0.17 (0341a63, Suite 1231/1231, nächstes Auto-Update): ImageRotationWorker lädt nach lokaler Retention hoch, löscht lokal NUR nach SHA-Bestätigung (Lösch-Gate; Fehlschlag → unangetastet, Retry täglich), .bgr weiterhin nur gelöscht; auto-aktiv bei CloudSync Enabled+Token (Merseburg: null Konfig nötig), Opt-out Botmatiq:Vision:ArchiveToCloud=false; vision_captures additiv ImageArchivedAt+ArchivePath; Wannen-PDF zeigt „im Langzeitarchiv gesichert (über Capture-ID abrufbar)", DocStorage-Texte beider Belege auf den Archiv-Weg umformuliert (alter „keine Cloud"-Satz wäre falsch geworden). Merke: Erste Uploads ~Ende August (30-Tage-Retention), Kontrolle via vision-archive/stats. Test-Gotcha: InMemory-DB-Name NIE inline im Options-Lambda erzeugen (wird je Kontext neu ausgewertet → jeder Scope eigene DB, Seed „verschwindet") — Name vorher in Variable fixieren.

## 2026-08-05 — VERBINDLICH: Keine Attribution-Trailer in Commits (Vorgabe Andreas)

Commits in allen Repos sind durchgängig Werk von Andreas Sawall/TECMATIQ GmbH: KEINE Co-Authored-By-/Claude-Session-Zeilen mehr, Author/Committer = Andreas Sawall <andysawall@gmail.com>. Bestand rückwirkend bereinigt (filter-branch --msg-filter, botmatiq 35 + .github 10 Commits, Tags via --tag-name-filter cat mitgezogen). Verfahren für künftige History-Eingriffe: Tag-getriggerte Workflows (windows-publish, ship-update-bundle, ops-deploy-superadmin) VOR dem Tag-Force-Push per gh workflow disable stilllegen (sonst CI-Gewitter + Bundle-mtime-Downgrade-Risiko: AllInOne zieht das NEUESTE ZIP nach mtime!), Branch-Protection main temporär allow_force_pushes=true (gh api PUT, Backup vorher), danach beides zurück. SHAs ab 29.07. verschoben — alte SHA-Notizen in worklog/Docs sind historisch. Gotcha: sed-Filter NIE inline durch die bash-c-Quoting-Kaskade reichen (Pattern kam als Literal-\" an, Lauf wirkte nur scheinbar) — Filter als Datei (/tmp/strip-trailers.sed) + sed -f; Verify NIE mit git log --all (zählt refs/original-Backups mit), sondern gegen main/Tags.

## 2026-08-05 — v4.0.19: AMOR-Vision-Schutz ±5%, Drop-Transparenz, Kiosk-Auto-Refresh (Poppitz-Vorfall)

Vorfall: „2 AMOR-Dateien nicht abgeholt" — Diagnose: WURDEN verarbeitet (13:44 + 13:54, ARTIKEL.ART 2 neu/2096 akt., BARCODE 891 Brücke, ARTFACH 824); Anruf fiel ins Update-Fenster 13:19–13:21. processed/ vom Klinik-Share-Aufräumjob gelöscht → update419 zieht ProcessedFolder/ErrorFolder per .env auf den IPC (C:\Botmatiq\amor\…). Root-Causes behoben (aefc449, Suite 1253/1253): (1) Feld-Hoheit präzisiert — Vision-Maße bleiben führend solange jede Dimension ±5% zum vollständigen AMOR-Maß (VisionDimensionTolerance=0.05); >5% ⇒ AMOR/manuell Datenhoheit (WARN je Artikel + Summenzeile); Teilmaße verdrängen Vision nie; Bilder/Vision-Datenquellen fasst der Import NIE an (Guard-Test); Gewicht folgt AMOR. (2) Unbekannte Drop-Dateien: benannte WARN statt lautlos; instabile/gelockte Dateien WARN nach ~1 min. (3) AmorOpsController: GET drop-status (Klassifikation+Locks+processed/error 24h), POST ingest-file (Supervisor; Umbenennen auf erwarteten Namen → EIN Scan-Pfad, 409 bei Kollision). (4) Kiosk: GET /api/v1/version (anonym) + KioskUpdateWatcher (60s-Poll, Auto-Reload nach Update, Schleifen-Schutz) + GET articles/data-version mit Auto-Refresh der Artikelstamm-Seite (nur leere Suche, kein offener Dialog). Offen: diag3-Kennzahl HEUTE_amor_ueber_vision (13:44-Import lief unter alter Doktrin) — falls >0 Restore aus learn_measurements (Spalten am IPC verifiziert).
